...
import { Store, CheckCircle, Clock, XCircle, ArrowLeft } from 'lucide-react';
...
export default function BecomeStore({ userRole, latestRequest }: PageProps) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(UpgradeController.storeStore.url());
    };

    const isUpgraded = userRole && (userRole.code === 'store_owner' || userRole.code === 'admin');

    return (
        <GuestLayout>
            <Head title="Become a Store - SoukNet" />
                ...
                {isUpgraded && (
                    <div className="bg-[#0c0d12] border border-[#262930] rounded-2xl p-8 text-center space-y-6 shadow-2xl">
                        ...
                        <div className="space-y-2">
                            <h2 className="text-2xl font-extrabold text-white">Already Registered Store</h2>
                            <p className="text-sm text-[#8f9bba]">
                                Your account currently has the <strong className="text-white font-bold">{userRole?.en}</strong> role. You can access the console to manage products and orders.
                            </p>
                        </div>
                        <a 
                            href="/dashboard" 
                ...
                {!isUpgraded && latestRequest && latestRequest.status === 'pending' && (
                    <div className="bg-[#0c0d12] border border-[#262930] rounded-2xl p-8 text-center space-y-6 shadow-2xl">
                        ...
                        <div className="space-y-2">
                            ...
                            <p className="text-sm text-[#8f9bba]">
                                Your request to become a Store Owner is currently being reviewed by our administrators.
                            </p>
                ...
                {!isUpgraded && (!latestRequest || latestRequest.status !== 'pending') && (
                    <div className="space-y-8">
                        ...
                        <div className="space-y-2 text-center md:text-left">
                            ...
                            <h1 className="text-3xl font-extrabold text-white tracking-tight">Become a Store</h1>
                            <p className="text-xs md:text-sm text-[#8f9bba] font-light">
                                Set up your store pipeline, publish products to your digital catalog, track inventory and fulfill incoming orders.
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="bg-[#0c0d12] border border-[#262930] rounded-2xl p-6 space-y-4 shadow-xl">
                            <div className="space-y-1.5">
                                <label className="text-xs font-bold text-[#8f9bba] uppercase tracking-wider">
                                    Introduce your store target catalog & inventory details
                                </label>
                                <textarea
                                    value={data.message}
                                    onChange={e => setData('message', e.target.value)}
                                    placeholder="Enter details about the products you wish to sell, your shipping/delivery setup, and active logistics..."
                                    rows={5}
                                    maxLength={1000}
